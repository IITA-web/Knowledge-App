...
static navigationOptions = ({ navigation, navigationOptions }) => {
    return {
      title: navigation.getParam('title', 'Photos'),

    headerRight: () => (
        <TouchableOpacity  style={{ paddingRight: 20}} onPress={navigation.getParam('searchIcon')}>
  ...
  componentDidMount(){
   ...
   //const isTablet = DeviceInfo.isTablet();
    this.setState({imagDimension: 100})
  }
...
const ImageDataArrays = 
                  [{
                      source: { uri: data.url_l },
                      title: data.title,
                      width: data.width_l,
                      height: data.height_l,
                  }];
this.setState({ImageDataArray: ImageDataArrays, isImageViewVisible: true});
}
  ...
  fetchPhotos(){
    ...
    fetch('https://www.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=f150eb83ccc18bfae28b269e0d2f6e0f&photoset_id='+itemId+'&user_id=45796762@N03&extras=date_upload,date_taken,last_update,geo,tags,views,media,url_m,url_l,url_o&per_page=30&format=json&nojsoncallback=1&page='+page)
      .then((response) => response.json())
      .then((responseJson) => {
        const photoData = responseJson['photoset']['photo'];
         var resdata = Object.keys(photoData).length;
         if (resdata > 0){
...
retrySearch = () =>{
  ...
  handleLoadMore = () => {
    ...
    this.setState(
      ...
      () => {
        if (this.state.searchActive == true && this.state.responseData > 0) {this.SearchPhotos(); }
        if (this.state.searchActive == false){this.fetchPhotos();}
        if (this.state.responseData < 30) {this.setState({loadingMore: false})}
      }
    );
  }
  };
 ...
 renderFooter = () => {
    ...
    return (
      ...
      >
        <ActivityIndicator animating color = '#222' size="large" ></ActivityIndicator>
      </View>
    );
  };
  ...
  render(){
    ...
    return  (
      ...
      spacing={1}
        ...
        <View>
          <View>
            ...
            <Image source={{ uri: item.url_m }} style={{width: '100%', height: 110}}/> 
            </TouchableOpacity>
          </View>
      ...
      <View>
              <ImageView
                  glideAlways
                  images={this.state.ImageDataArray}
                  animationType="fade"
                  isVisible={isImageViewVisible}
                  renderFooter={this.Footers}
